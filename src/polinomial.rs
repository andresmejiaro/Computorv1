...
impl Polinomial {
    ...
    pub fn simplify_grade(&mut self) {
        if self.coefficients.ends_with(&[0.0]) && self.coefficients.len() > 1 {
    ...
    fn solver_msg2(&self) {
        ...
        if discr < 0.0 {
            ...
            println!(
                "{:.6} + {:.6}i",
                -0.5 * self.coefficients[1] / self.coefficients[2],
                impart / self.coefficients[2] / 2.
            );
            println!(
                "{:-.6} - {:.6}i",
                -0.5 * self.coefficients[1] / self.coefficients[2],
    ...
    fn high_level_solver(&self) {
        ...
        loop {
            ...
            println!(
                "the polynomial evaluated at that point is: {:.5}",
                self.evaluate(solved)
            );
        ...
        match self.degree() {
            0 => {
                if self.coefficients[0] != 0.0 {
                    println!("This equation is inconsistent. No value satisfies it");
                } else {
                    println!("This equation is a tautology. All values satisfy it");
                }
            }
            1 => {
