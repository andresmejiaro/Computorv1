...
fn poly_parser(textpoly: &str) -> Polinomial {
    if textpoly.trim().len() == 0 {
        println!("Empty polinomial");
        process::exit(1);
    }
    ...
    for mon in monomials_text {
        match mon {
            ...
            "+" => {
                if parser_state.caret_appeared
                    || parser_state.mult_apeared
                    || parser_state.x_apeared
                    || parser_state.coef_parsed
                {
...
fn update_polynomial(parser_state: &ParserState, polinomial: &mut Polinomial) {
    polinomial.update_coef(
        parser_state.coef * parser_state.coef_sign,
        parser_state.power,
    );
}
